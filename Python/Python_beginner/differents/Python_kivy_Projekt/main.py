...
# register Label form size
class ResizableLabel2(Label):
    max_font_size = 11  # Define your maximum font size here

    def on_width(self, instance, value):
        if value < self.size[0]:
            self.font_size = max(value * 2, self.max_font_size)
        else:
            self.font_size = self.max_font_size
...
class MyApplicationApp(App):
    ...
    def get_input_value(self):
        ...
        input_values_list['password'] = self.root.ids.password.text
        password_input = input_values_list['password']

        input_values_list['re_password'] = self.root.ids.re_password.text
        re_password = input_values_list['re_password']
        ...
        #print("Repeat Password: ", re_password)

        # new list asl dictionary
        new_dictoinary = {"Vor Name": v_name, "Nach Name": n_name, "email": email_addres, "Repeat Eamail": repeat_email, "Password": password_input,"Repeat Password": re_password }
        print("Here is a dictionary of inputs: ",new_dictoinary)

        # new list and add elelments into it
        new_list = []
        new_list.append(v_name)
        new_list.append(n_name)
        new_list.append(email_addres)
        new_list.append(repeat_email)
        new_list.append(password_input)
        new_list.append(re_password)
