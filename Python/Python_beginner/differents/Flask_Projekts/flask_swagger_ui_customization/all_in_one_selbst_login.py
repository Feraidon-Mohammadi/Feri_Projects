...
class Get_Odoo_Data:
    ...
    records3 = models.execute_kw(odoo_db_info['database'], uid, odoo_db_info['password'], model3, 'search_read',
                                 partner_ids, {
                                     'fields': ["id", "name", "measure_name", "education_end_date",
                                                "education_school_day_total"], "limit": 2})



# Define a model for the Odoo data
odoo_model = api.model("get_odoo_data", {
    "Maßnahmenummer":fields.__all__
})
...
class OdooDataResource(Resource):
    ...
    def get(self):
        ...
        modified_data = [
            {
                'Maßnahmenummer': item.get('maßnahmenummer'),
                'startdatum': item.get('startdatum'),
                'enddatum': item.get('enddatum'),
            }
            for item in x
        ]
...
#         return f"JWT is verified for user: {username}. Welcome to my page."
#     else:
...
#     #raw_jwt = get_jwt() # for test to see client geting response or not
...
def login():
    if request.method == "GET":
        ...
        if user:
            ...
            x = jsonify(access_token=access_token, message="Login successful")
...
#             return make_response("Unable to verify", 401, {"WWW-Authenticate": "Bearer realm='Authentication Failed!'"})
...
# @jwt_required()
# @login_required
# @app.route('/swagger', endpoint="swagger")
# # @login_required
# def swagger_ui():
#     verify_jwt_in_request()
#     #access_token = create_access_token(identity='username')
