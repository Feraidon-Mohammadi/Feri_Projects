...
app.config['SECRET_KEY'] = "5e884898da28047151d0e56f8dc6292773603d0d6aabbdd62a11ef721d1542d8"  # Add this line to set the secret key
app.config['JWT_SECRET_KEY'] = '447fce4bbc4b8f6da86714945d33fe0590d55e8d90fd1d526a68c774ea9f2d70' # jwt secrete key

#response = requests.get('https://your-api/protected-route', headers=headers)



# token = jwt.encode(paylaod = my_jwt_token,
#                    key = "my_secret_key"
#                    )
# print(my_jwt_token)
############################################### token ersteleln #####################################
...
# import requests
#
# # Replace '<your_jwt_token>' with the actual JWT token you received during authentication
# jwt_token = '<your_jwt_token>'
#
# # Replace the URL with the actual URL of your protected route
...
model_new = "de_iad_tm.measure"
suchfeld = "Maßnahmenummer: number"
ruckgabe = "special_day_ids"
in_special_days = "in Special_days gibt es: date,  type, note"
...
class Get_Odoo_Dta:
    ...
    limit = 1
    partner_ids = models.execute_kw(odoo_db_info['database'], uid, odoo_db_info['password'], model, 'search_read', domain)

    records1 = models.execute_kw(odoo_db_info['database'], uid, odoo_db_info['password'], model, 'search_read', partner_ids,
                                 {'fields': ['id', 'display_name', 'vorname', 'nachname', 'forename', 'name', "city", "street", "phone"]})
...
@ns.route('/odoo data1/<string:forename>/<string:display_name>/<int:id>')
# @ns.route('/odoo data/<int:id>')
class OdooDataResource(Resource):
    ...
    def get(self, forename, display_name, id):
        # Fetch Odoo data
        x = Get_Odoo_Dta.records1
        filtered_data = [item for item in x if item.get('forename') == forename or item.get('display_name') == display_name
                         or item.get("id") == id]
